#!/usr/bin/env python3
"""
Validate PaleoCode coverage against cuneiform signs glossary.

This script checks how many signs from the cuneiform_signs.csv glossary
can be encoded using the PaleoCode system.
"""
...
def validate_paleocode_coverage(
    glossary_path: str,
    output_file: str = None,
    verbose: bool = False
) -> Dict:
    """
    Validate PaleoCode coverage against glossary.
    ...
    """
    print("=" * 70)
    print("PALEOCODE COVERAGE VALIDATION")
    print("=" * 70)

    # Load PaleoCode converter
    print("\nLoading PaleoCode database...")
    converter = PaleoCodeConverter()

    # Get all signs in PaleoCode
    paleocode_signs = set(converter.get_all_unicode_signs())
    print(f"PaleoCode database contains {len(paleocode_signs)} signs")
    ...
    # Find coverage
    covered_signs = glossary_signs & paleocode_signs
    missing_signs = glossary_signs - paleocode_signs
    extra_signs = paleocode_signs - glossary_signs
    ...
    results = {
        ...
        'paleocode_total': len(paleocode_signs),
    ...
    print(f"Total signs in PaleoCode:       {results['paleocode_total']:>6}")
    print(f"Signs covered by PaleoCode:     {results['covered']:>6}")
    print(f"Signs missing from PaleoCode:   {results['missing']:>6}")
    print(f"Extra signs in PaleoCode:       {results['extra']:>6}")
    ...
    if missing_signs:
        ...
        print("=" * 70)
        print("(Signs in glossary but not in PaleoCode database)")
        print()
    ...
    if extra_signs:
        print("\n" + "=" * 70)
        print(f"EXTRA SIGNS IN PALEOCODE ({len(extra_signs)} signs)")
        print("=" * 70)
        print("(Signs in PaleoCode but not in glossary)")
        print()
        ...
        if verbose:
            for idx, sign in enumerate(sorted(extra_signs), 1):
                ...
                print(f"{idx:4}. {sign}  {code_point:>8}  PaleoCode: {paleocode}")
        else:
            # Just show first 20
    ...
    for idx, sign in enumerate(sorted(list(covered_signs)[:10]), 1):
        ...
        print(f"{idx:2}. {sign}  {code_point:>8}  →  {paleocode:20}  ({phonetic})")
    ...
    if output_file:
        ...
        detailed_report = {
            'summary': {
                ...
                'paleocode_total': results['paleocode_total'],
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate PaleoCode coverage against cuneiform glossary"
    )
    parser.add_argument(
        "--glossary",
        type=str,
    ...
    # Run validation
    results = validate_paleocode_coverage(
    ...
    if results['missing'] > 0:
        print(f"\n⚠️  Warning: {results['missing']} signs from glossary are not in PaleoCode")
        print(f"Coverage: {results['coverage_percent']:.2f}%")
    else:
        print(f"\n✓ All glossary signs are covered by PaleoCode!")
